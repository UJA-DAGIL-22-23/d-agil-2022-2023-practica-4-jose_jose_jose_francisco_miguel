...
describe("Waterpolo", function() {
  describe("listarTodosLosDatos", function() {
    beforeEach(function() {
      // Configurar el entorno necesario para las pruebas
      jasmine.getFixtures().fixturesPath = 'path/to/fixtures/directory';
      loadFixtures('index.html');
    });

    afterEach(function() {
      // Restaurar el entorno después de las pruebas
      jasmine.getFixtures().cleanUp();
    });

    it("debería mostrar los datos correctamente", function() {
      // Configurar el vector de datos de prueba
      var vector = [
        {
          ref: { '@ref': { id: '1' } },
          data: {
      ...
      Waterpolo.listarTodosLosDatos(vector);

      // Verificar que los elementos correspondientes estén ocultos
      expect(porCampo.style.display).toEqual('none');
      expect(porNombre.style.display).toEqual('none');
      expect(porMinimoUnCriterio.style.display).toEqual('none');
      expect(porCuatroCriterios.style.display).toEqual('none');

      // Verificar que los datos se muestren correctamente en el artículo
      var expectedCabecera = '<table width="100%" class="listado-personas-SoloNombres"><thead><th width="10%">Id</th><th width="20%">Nombre</th><th width="20%">Apellidos</th><th width="10%">fecha nacimiento</th><th width="15%">Competiciones</th><th width="15%">Nacionalidad</th><th width="15%">Peso</th><th width="15%">Posición</th><th width="10%">Acciones</th></thead><tbody>';
      var expectedPie = '</tbody></table>';

      var expectedPersona = '<tr><td>1</td><td>John</td><td>Doe</td><td>1/1/1990</td><td>Liga, Copa</td><td>España</td><td>80kg</td><td>Portero</td><td></td></tr>';
      var expectedMensaje = expectedCabecera + expectedPersona + expectedPie;

      expect(Frontend.Article.actualizar).toHaveBeenCalledWith("Listado de todos los datos de todos/as los/as jugadores/as", expectedMensaje);
    });
  });
});
