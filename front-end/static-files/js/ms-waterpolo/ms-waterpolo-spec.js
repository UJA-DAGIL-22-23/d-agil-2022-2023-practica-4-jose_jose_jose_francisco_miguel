...
const TITULO_HOME = "Waterpolo Home"
const TITULO_ACERCA_DE = "Waterpolo Acerca de"
...
describe("Waterpolo.mostrarAcercaDe: ", function () {
  it("muestra datos nulos cuando le pasamos un valor nulo",
    function () {
      ...
      expect(elementoContenido.innerHTML.search(Waterpolo.datosDescargadosNulos.mensaje) >= 0).toBeTrue()
    })
...
describe('Waterpolo', function () {
  describe('.sustituyeTagsTodosLosDatosSoloNombres', function () {
    it('debe reemplazar todas las etiquetas de nombre de la Waterpolo con el nombre de la persona', function () {
      // Definimos la Waterpolo y la persona
      var Waterpolo = 'Hola, mi nombre es Juan';
      var persona = {
        data: {
          nombre: 'Juan'
        }
      ...
      var resultado = Waterpolo.sustituyeTagsTodosLosDatosSoloNombres(Waterpolo, persona);

      // Verificamos que la función haya hecho el reemplazo correctamente
      expect(resultado).toEqual('Hola, mi nombre es Juan');
    });

    it('debe reemplazar todas las etiquetas de nombre de la Waterpolo con el nombre de la persona, incluso si la Waterpolo tiene varias etiquetas', function () {
      // Definimos la Waterpolo y la persona
      var Waterpolo = 'Hola, Pedro. Soy Pedro y quiero hablar contigo, Pedro!';
      var persona = {
        data: {
          nombre: 'Pedro'
        }
...
describe("Waterpolo.imprimeSoloNombres", function () {
  ...
  it("debería ordenar correctamente el vector de objetos por nombre y mostrar el listado en la tabla", function () {
    ...
    let expectedTable = Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.cabecera
      + Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.actualiza(vector[0])
      + Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.actualiza(vector[1])
      + Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.actualiza(vector[2])
      + Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.pie;
...
describe('Waterpolo.imprimeCuatroCriterios', function () {
  ...
  it("Muestra un listado de jugadores/as con todos los datos",
    function () {
      const expectedMsj = Waterpolo.WaterpoloTablaPersonasTodosLosDatosSINID.cabecera + Waterpolo.cuerpoTr(vector[0]) + Waterpolo.cuerpoTr(vector[1]) + Waterpolo.WaterpoloTagsTodosLosDatos.pie;
      spyOn(Frontend.Article, 'actualizar');
...
describe("Waterpolo.sustituyeTagsTodosLosDatos", function () {
  it("debería reemplazar todas las etiquetas con los datos de la persona", function () {
    const Waterpolo = "ID: abc123, Nombre: Juan, Apellidos: Pérez, Nacionalidad: Española, Peso: 75 kg, Posición: Delantero, Fecha de nacimiento: 1/1/1990";
    const persona = {
      ref: { "@ref": { id: "abc123" } },
      data: {
        nombre: "Juan",
    ...
    const resultadoObtenido = Waterpolo.sustituyeTagsTodosLosDatos(Waterpolo, persona);

    expect(resultadoObtenido).toEqual(resultadoEsperado);
  });
});

describe('Waterpolo.WaterpoloTablaPersonasTodosLosDatos.actualiza', function () {
  let Waterpolo;

  beforeEach(function () {
    // Creamos una Waterpolo de ejemplo para cada test
    Waterpolo = '<tr><td>ID</td><td>NOMBRE</td><td>APELLIDOS</td><td>NACIONALIDAD</td><td>PESO</td><td>POSICION</td><td>FEC_NAC</td></tr>';
  });

  it('Debería reemplazar todas las etiquetas de la Waterpolo por los datos de la persona', function () {
    ...
    const resultado = Waterpolo.WaterpoloTablaPersonasTodosLosDatos.actualiza(persona);
...
describe('Waterpolo.jugador', function () {
  ...
  beforeEach(function () {
    spyOn(Waterpolo.WaterpoloFormularioUnJugador, 'actualiza').and.returnValue('<p>Información del jugador</p>');
    spyOn(Frontend.Article, 'actualizar');
  });

  it('debe llamar a Waterpolo.WaterpoloFormularioUnJugador.actualiza con el/las jugador/a como parámetro', function () {
    Waterpolo.jugador(jugador);
    expect(Waterpolo.WaterpoloFormularioUnJugador.actualiza).toHaveBeenCalledWith(jugador);
  });
...
describe('Waterpolo', function () {
  it('debería actualizar la Waterpolo y el artículo', function () {
    var jugador = { nombre: 'Juan', edad: 25 };
    var msj = 'Mensaje de prueba';

    spyOn(Waterpolo.WaterpoloFormularioUnJugador, 'actualiza').and.returnValue(msj);
    spyOn(Frontend.Article, 'actualizar');
...
describe('Waterpolo.anterior', function () {
  ...
  it('muestra el formulario vacío si el vector tiene un solo elemento', function () {
    ...
    expect(Waterpolo.sustituyeTagsTodosLosDatos).toHaveBeenCalledWith(Waterpolo.WaterpoloFormularioUnJugador.formulario);
  });
