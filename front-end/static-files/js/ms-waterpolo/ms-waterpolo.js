/**
 * @file Waterpolo.js
 * @description Funciones para el procesamiento de la info enviada por el MS Waterpolo
...
// Waterpolo de datosDescargados vacíos
...
Waterpolo.WaterpoloTagsSoloNombres = {
    "NOMBRE": "### NOMBRE ###",
}


// Waterpolo para poner los datos de varias personas dentro de una tabla
Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres = {}

// Cabecera de la tabla
Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.cabecera = `<table width="100%" class="listado-personas-SoloNombres">
                    <thead>
...
Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.cuerpo = `
    <tr title="${Waterpolo.WaterpoloTagsSoloNombres.ID}">
        <td>${Waterpolo.WaterpoloTagsSoloNombres.NOMBRE}</td>
        </td>
    </tr>
    `;

// Pie de la tabla
Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.pie = `        </tbody>
             </table>
             `;

/**
 * Actualiza el cuerpo de la Waterpolo deseada con los datos de la persona que se le pasa
 * @param {String} Waterpolo Cadena conteniendo HTML en la que se desea cambiar lso campos de la Waterpolo por datos
 * @param {Persona} Persona Objeto con los datos de la persona que queremos escribir en el TR
 * @returns La Waterpolo del cuerpo de la tabla con los datos actualizados 
 */
Waterpolo.sustituyeTagsTodosLosDatosSoloNombres = function (Waterpolo, persona) {
    return Waterpolo
        .replace(new RegExp(Waterpolo.WaterpoloTagsSoloNombres.NOMBRE, 'g'), persona.data.nombre)
}
 ...
 * @returns La Waterpolo del cuerpo de la tabla con los datos actualizados 
 */
Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.actualiza = function (persona) {
    return Waterpolo.sustituyeTagsTodosLosDatosSoloNombres(this.cuerpo, persona)
}

/**
 * Función que descarga la info MS Waterpolo al llamar a una de sus rutas
...
Waterpolo.descargarRuta = async function (ruta, callBackFn) {
    ...
    // Intento conectar con el microservicio Waterpolo
    try {
...
/**
 * Función principal para mostrar los datos enviados por la ruta "home" de MS Waterpolo
 */
Waterpolo.mostrarHome = function (datosDescargados) {
    ...
    Frontend.Article.actualizar("Waterpolo Home", datosDescargados.mensaje)
}

/**
 * Función principal para mostrar los datos enviados por la ruta "acerca de" de MS Waterpolo
 */
Waterpolo.mostrarAcercaDe = function (datosDescargados) {
    ...
    `;
    Frontend.Article.actualizar("Waterpolo Acerca de", mensajeAMostrar)
}
...
Waterpolo.imprimeSoloNombres = function (vector) {
    ...
    let msj = Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.cabecera
    vector.forEach(e => msj += Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.actualiza(e))
    msj += Waterpolo.WaterpoloTablaPersonasTodosLosDatosSoloNombres.pie
...
Waterpolo.WaterpoloTagsTodosLosDatos = {
    "ID": "### ID ###",
    "NOMBRE": "### NOMBRE ###",
    "APELLIDOS": "### APELLIDOS ###",
    "FEC_NAC": "### FEC_NAC ###",
...
/// Waterpolo para poner todos los datos de todos/as los/as jugadores/as dentro de una tabla
Waterpolo.WaterpoloTablaPersonasTodosLosDatos = {}

/// Waterpolo para poner todos los datos de todos/as los/as jugadores/as dentro de una tabla
Waterpolo.WaterpoloTablaPersonasTodosLosDatosSINID = {}

// Cabecera de la tabla que muestra todos los datos de todos/as los/as jugadores/as
Waterpolo.WaterpoloTablaPersonasTodosLosDatos.cabecera = `<table width="100%" class="listado-personas-SoloNombres">
                    <thead>
                        <th width="10%">Id</th>
...
// Cabecera de la tabla que muestra todos los datos de todos/as los/as jugadores/as
Waterpolo.WaterpoloTablaPersonasTodosLosDatosSINID.cabecera = `<table width="100%" class="listado-personas-SoloNombres">
<thead>
    <th width="20%">Nombre</th>
    <th width="20%">Apellidos</th>
    <th width="10%">fecha nacimiento</th>
...
Waterpolo.WaterpoloTablaPersonasTodosLosDatos.cuerpo = `
    <tr title="${Waterpolo.WaterpoloTagsTodosLosDatos.ID}">
        <td>${Waterpolo.WaterpoloTagsTodosLosDatos.ID}</td>
        <td>${Waterpolo.WaterpoloTagsTodosLosDatos.NOMBRE}</td>
        <td>${Waterpolo.WaterpoloTagsTodosLosDatos.APELLIDOS}</td>
        <td>${Waterpolo.WaterpoloTagsTodosLosDatos.FEC_NAC}</td>
        <td>${Waterpolo.WaterpoloTagsTodosLosDatos.COMPETICIONES}</td>
        <td>${Waterpolo.WaterpoloTagsTodosLosDatos.NACIONALIDAD}</td>
        <td>${Waterpolo.WaterpoloTagsTodosLosDatos.PESO}</td>
        <td>${Waterpolo.WaterpoloTagsTodosLosDatos.POSICION}</td>
        <td><div><a href="javascript:Waterpolo.mostrar('${Waterpolo.WaterpoloTagsTodosLosDatos.ID}')" class="opcion-principal mostrar">Mostrar Jugador/a</a></div></td>
    </tr>
    `;

// Pie de la tabla de todos los datos de todos/as los/as jugadores/as
Waterpolo.WaterpoloTablaPersonasTodosLosDatos.pie = `        </tbody>
             </table>
             `;

/**
 * Actualiza el cuerpo de la Waterpolo deseada con los datos del jugador/as que se le pasa
 * @param {String} Waterpolo Cadena conteniendo HTML en la que se desea cambiar los campos de la Waterpolo por datos
 * @param {Persona} Persona Objeto con los datos del jugador/as que queremos escribir en el TR
 * @returns La Waterpolo del cuerpo de la tabla con los datos actualizados 
 */
Waterpolo.sustituyeTagsTodosLosDatos = function (Waterpolo, persona) {
    return Waterpolo
        .replace(new RegExp(Waterpolo.WaterpoloTagsTodosLosDatos.ID, 'g'), persona.ref['@ref'].id)
        .replace(new RegExp(Waterpolo.WaterpoloTagsTodosLosDatos.NOMBRE, 'g'), persona.data.nombre)
        .replace(new RegExp(Waterpolo.WaterpoloTagsTodosLosDatos.APELLIDOS, 'g'), persona.data.apellidos)
        .replace(new RegExp(Waterpolo.WaterpoloTagsTodosLosDatos.NACIONALIDAD, 'g'), persona.data.nacionalidad)
        .replace(new RegExp(Waterpolo.WaterpoloTagsTodosLosDatos.PESO, 'g'), persona.data.peso)
        .replace(new RegExp(Waterpolo.WaterpoloTagsTodosLosDatos.POSICION, 'g'), persona.data.posicion)
        .replace(new RegExp(Waterpolo.WaterpoloTagsTodosLosDatos.COMPETICIONES, 'g'), persona.data.competiciones)
        .replace(new RegExp(Waterpolo.WaterpoloTagsTodosLosDatos.FEC_NAC, 'g'), persona.data.fec_nac.dia + "/" + persona.data.fec_nac.mes + "/" + persona.data.fec_nac.anio)
}
 ...
 * @returns La Waterpolo del cuerpo de la tabla con los datos actualizados 
 */
Waterpolo.WaterpoloTablaPersonasTodosLosDatos.actualiza = function (persona) {
...
Waterpolo.listarTodosLosDatos = function (vector) {
    ...
    let msj = Waterpolo.WaterpoloTablaPersonasTodosLosDatos.cabecera
    vector.forEach(e => msj += Waterpolo.WaterpoloTablaPersonasTodosLosDatos.actualiza(e))
    msj += Waterpolo.WaterpoloTablaPersonasTodosLosDatos.pie
...
Waterpolo.imprimePor = function (campo, vector) {
    ...
    let msj = "";
    msj += Waterpolo.WaterpoloTablaPersonasTodosLosDatosSINID.cabecera;
...
Waterpolo.jugador = function (jugador) {
    ...
    let msj = Waterpolo.WaterpoloFormularioUnJugador.actualiza(jugador)
    Frontend.Article.actualizar("Jugador/a elegido/a", msj)
    return msj;
}

/// Waterpolo para poner los datos de un/a jugado/a
Waterpolo.WaterpoloFormularioUnJugador = {}

// Formulario para mostrar los datos de un/a jugador/a
Waterpolo.WaterpoloFormularioUnJugador.formulario = `
<form method='post' action=''>
    ...
    <a href="javascript:Waterpolo.jugadorSiguiente()"  class="opcion-principal mostrar">Siguiente</a>
        ...
        <tbody>
            <tr title="${Waterpolo.WaterpoloTagsTodosLosDatos.ID}">
                <td><input type="text" class="form-persona-elemento" disabled id="idJugador" value="${Waterpolo.WaterpoloTagsTodosLosDatos.ID}" name="id"/></td>
                <td>${Waterpolo.WaterpoloTagsTodosLosDatos.NOMBRE}</td>
                <td>${Waterpolo.WaterpoloTagsTodosLosDatos.APELLIDOS}</td>
                <td>${Waterpolo.WaterpoloTagsTodosLosDatos.FEC_NAC}</td>
                <td>${Waterpolo.WaterpoloTagsTodosLosDatos.COMPETICIONES}</td>
...
/**
 * Función que actualiza un/a jugador/as en la Waterpolo del formulario
 * @param {*} jugador La información del jugador/a que queremos mostrar
 * @returns 
 */
Waterpolo.WaterpoloFormularioUnJugador.actualiza = function (jugador) {
...
Waterpolo.siguiente = function (vector) {
    ...
    if (typeof pos === "number") {
        ...
        Waterpolo.sustituyeTagsTodosLosDatos(Waterpolo.WaterpoloFormularioUnJugador.formulario)
    return indices
}
 ...
 * @param {Vector_de_persona} vector Vector con los datos de los Waterpolo a mostrar
 */
Waterpolo.imprime = function (vector) {
    let msj = "";
    msj += Waterpolo.WaterpoloTablaPersonasTodosLosDatosSINID.cabecera;
    vector.forEach(e => msj += Waterpolo.cuerpoTr(e))
    msj += Waterpolo.WaterpoloTagsTodosLosDatos.pie;
