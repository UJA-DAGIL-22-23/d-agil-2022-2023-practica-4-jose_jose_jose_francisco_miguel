...
Deportistas.botones=`<h1>Aplicación Microservicios Remo</h1>
<div>
...
Deportistas.modificarDeportista = async function (id) {
    try {
        let url = Frontend.API_GATEWAY + "/remo/setDeportista/" + id
        const response = await fetch(url, {
            method: 'POST', // *GET, POST, PUT, DELETE, etc.
            mode: 'no-cors', // no-cors, cors, *same-origin
            cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
...
Deportistas.recuperaUnDeportista = async function (id, callBackFn) {
    try {
        const url = Frontend.API_GATEWAY + "/remo/deportista/" + id
        const response = await fetch(url);
        if (response) {
